<?php
	...
	if(!isset($_GET["canvas"])) die("Canvas id not set");
	
	$user_id = $_GET["user"];
	$canvas_id = $_GET["canvas"];
?>
...
<html>
	...
	<body>
		<!-- Needs to be merged with main css -->
		<style>
		...
		<script>
			...
			function insert_data() {
					//PUT -d "{\"a\":\"b\"}" http://127.0.0.1:5984/wiki/foo
				$.ajax({
    				type: "PUT",
    				url: "http://localhost:5984/wiki/foo2",
    				contentType: "application/json",
    				data: {"data": "{\"a\":\"b\"}"}
				});
				
				// var db = new CouchDB("sample", {"X-Couch-Full-Commit":"false"});
				// db.save({_id:"0",a:1,b:1});
				// db.save({_id:"2",a:2,b:3});
			}
			
		</script>
		<input type="button" value="test insert" onclick="insert_data()"/>
